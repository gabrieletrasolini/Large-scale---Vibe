...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['track_id']) && !empty($_POST['track_name'])) {
    ...
    try {
        $q      = new MongoDB\Driver\Query(['username' => $utente, 'preferiti.brani.id_brano' => $trackId]);
        $c      = $manager->executeQuery('admin.User', $q);
        $esiste = iterator_count($c) > 0;
        if ($esiste) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'get_playlists') {
    ...
    $q          = new MongoDB\Driver\Query(['username' => $username]);
    $userCursor = $manager->executeQuery('admin.User', $q);
    $user       = current($userCursor->toArray());
    if (!$user) { die('Utente non trovato'); }
    header('Content-Type: application/json');
    if (!empty($user->playlist_personali)) {
        $playlistArray = array_map(function($pl) {
            return [
                'nome_playlist' => $pl->nome_playlist,
                'descrizione'   => $pl->descrizione ?? '',
                'brani'         => array_map(fn($b) => $b->id_brano, $pl->brani ?? [])
            ];
...
// ──────────────────────────────────────────────
//  Filtro base: canzoni dell'artista
//  + eventuale ricerca per titolo + sort
// ──────────────────────────────────────────────
$filters = ['Artist_Name' => ['$regex' => '^' . preg_quote($artistName, '/') . '$', '$options' => 'i']];
...
<head>
    ...
    <title><?= htmlspecialchars(ucwords($artistName)) ?> — Songs</title>
    ...
    <style>
        ...
        /* ── Toast ── */
        .toast {
            visibility: hidden; min-width: 250px; margin-left: -155px;
            background-color: #333; color: white; text-align: center;
            border-radius: 2px; padding: 16px; position: fixed;
            z-index: 9999; left: 50%; bottom: 30px; font-size: 17px;
        }
        ...
        .back-btn:hover { background: rgba(255,255,255,0.28); color: white; text-decoration: none; }
...
<body class="main-layout about-page">
    ...
    <div class="sfondo">
        ...
        <!-- Titolo e back button -->
        <div style="text-align:center; padding-top: 20px;">
            <h1 style="color: white; font-weight: bold; margin-bottom: 8px;">
                🎤 <?= htmlspecialchars(ucwords($artistName)) ?>
            </h1>
            <p style="color: #ccc; font-size: 15px; margin-bottom: 14px;">
                <?= $totalSongs ?> song<?= $totalSongs != 1 ? 's' : '' ?> found
            </p>
            <a href="Artist.php" class="back-btn">← Back to Artists</a>
        </div>

        <!-- ════════════ FILTRI ════════════ -->
        <form method="GET" class="formfiltri">
            <!-- Preserva il nome artista -->
        ...
        <div class="song-list">
            ...
            <?php if (empty($uniqueTracks)): ?>
            <div style="text-align:center; color: white; padding: 40px 0; font-size: 18px;">
                No songs found for <strong><?= htmlspecialchars(ucwords($artistName)) ?></strong>.
            </div>
            <?php endif; ?>
        </div>
        <!-- end song list -->
        ...
        <div id="lyrics-overlay" class="popup-overlay" style="display: none;">
            <div class="popup-content" style="max-width: 620px; width: 92%;">
                <h2 id="lyrics-title" style="font-size:18px; margin-bottom:4px;"></h2>
        ...
        <div id="features-overlay" class="popup-overlay" style="display: none;">
            <div class="popup-content" style="max-width: 500px; width: 92%;">
                <h2 id="features-title" style="font-size:18px; margin-bottom:4px;"></h2>
                <div class="overlay-scroll">
